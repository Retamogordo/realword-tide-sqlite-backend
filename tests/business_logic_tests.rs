...
async fn tests() -> Result<(), errors::BackendError> {
    ...
    print!("registering user {}...", reg_scott_smith.username);
    ...
    /*    match res { 
        Ok(_) 
        |
        Err(errors::BackendError::UsernameOrEmailExists) => (),
    ...
    print!("logging in user with email {}...", login_scott_smith.email);
    ...
    print!("logging in user with incorrect password...");
    ...
    print!("logging in non-existing user...");
    ...
    print!("trying to register existing user {}...", reg_scott_smith.username);
    server.register_user(reg_scott_smith).await
        .expect_err("Registered user with username or email already taken.");
    println!(" failed as expected.");

    // current user by token
    print!("getting user by JWT token ...");
    ...
    println!(" done, username: {}.", scott_smith_current.username);
    ...
    print!("registering user {}...", reg_james_joyce.username);
    ...
    print!("updating users {} profile...", james_joyce_logged_in.username);
    ...
    println!(" done, updated bio: {}", profile_james_joyce.bio);

    //
    print!("retrieving non-existing users profile...");
    server.profile("graham_green").await.expect_err("Got profile of non-existing user.");
    println!(" failed as expected.");
    
    // smith follows joyce
    print!("{} follows user {} ...", scott_smith_logged_in.username, james_joyce_logged_in.username);
    ...
    println!(" done, followed user is {}.", profile_james_joyce.username);
    ...
    print!("{} creates an article ...", james_joyce_logged_in.username);
    ...
    println!(" done, title: {}.", article_response1.article.title);
    ...
    let article_response2 = server.create_article(
        james_joyce_logged_in.token.as_ref().unwrap(), 
        create_article).await?;
    println!(" done, title: {}.", article_response2.article.title);
    ...
    println!(" done, they are: {}.", tags);
    
    // check if articles are in db
    print!("checking number of registered articles...");
    ...
    print!("registering user {}...", reg_graham_greene.username);
    ...
    print!("{} creates an article ...", graham_greene_logged_in.username);
    let article_response3 = server.create_article(
        graham_greene_logged_in.token.as_ref().unwrap(), 
        create_article
    ).await?;
    println!(" done, title: {}.", article_response3.article.title);

    // scott smith tries to favorite article by greene
    let slug ="the-quiet-american";
    print!("{} favorites an article {}...", scott_smith_logged_in.username, slug);
    ...
    print!("{} favorites article {}...", james_joyce_logged_in.username, slug);
    ...
    let slug = "the-quiet-american";
    print!("{} unfavorites article {}...", james_joyce_logged_in.username, slug);
    ...
    print!("{} follows user {}...", scott_smith_logged_in.username, graham_greene_logged_in.username);
    ...
    print!("feed articles for user {}...", scott_smith_logged_in.username);
    ...
    println!(" done, fed {} articles.", articles_fed_for_scott_smith.articles.len());
    ...
    print!("{} updates article {}...", graham_greene_logged_in.username, update_article_req.slug);
    ...
    print!("{} deletes article {}...", scott_smith_logged_in.username, "the-calm-american");
    ...
    print!("{} updates article {}...", scott_smith_logged_in.username, update_article_req.slug);    
    server.update_article(
        scott_smith_logged_in.token.as_ref().unwrap(), 
        update_article_req,
    )
    ...
    print!("{} deletes non-existing article ...", scott_smith_logged_in.username);
    server.delete_article(
        scott_smith_logged_in.token.as_ref().unwrap(), 
        "dummy",
    )
    ...
    print!("{} updates non-existing article ...", scott_smith_logged_in.username);
    server.update_article(
        graham_greene_logged_in.token.as_ref().unwrap(), 
        update_article_req,
    )
    ...
    print!("{} deletes a (not owned) comment to article {}...", 
        graham_greene_logged_in.username, delete_by.article_slug);
    server.delete_comment(
        graham_greene_logged_in.token.as_ref().unwrap(), 
        delete_by,
    )
