...
impl Server {
    ...
    pub fn secret(mut self, secret: &'static [u8]) -> Self {
    ...
    pub async fn register_user(&self, user_reg: UserReg) -> Result<User, BackendError> {
        ...
        db::user::register_user(&self.conn, &user_reg).await?;
    
        let mut user: User = user_reg.into();
        user.token = Some(auth::Auth::create_token(&user, self.secret.unwrap())?);
        Ok(user)
    }

    pub async fn login_user(&self, user_by: filters::UserFilter) -> Result<User, BackendError> {
        let mut user = db::user::get_user(&self.conn, user_by).await?;
...
/*        let res = match db::user::get_profile(&self.conn, username).await {
    ...
    pub async fn update_user(&self, token: &str, update_user: UserUpdate) -> Result<User, BackendError> {
        ...
        let mut user = db::user::update_user(&self.conn, &update_user, filter).await?;
        user.token = Some(token.to_string());
        Ok(user)
    }
    ...
    pub async fn get_articles(&self, 
        ...
        let articles = db::article::get_all(&self.conn, 
            article_by, 
            order_by, 
            limit_offset)
        .await?;
...
//        mut update_by: filters::UpdateArticleFilter<'_>,
    ...
    let res = match db::article::update_article(&self.conn, 
                                                update_article, 
                                                update_by).await {
        Ok(article_response) => {
            Ok(article_response)
        ...
        Err(err) => match err {
            ...
            crate::errors::BackendError::NoArticleFound => {
                ...
                if let Ok(article_response) = db::article::get_one(&self.conn, filter).await {

 //                   let token = crate::utils::token_from_request(&req)?;
 //                   let secret = req.state().secret;
                    auth::Auth::authorize(token, self.secret.unwrap(), &article_response.article.author)?;
    ...
    //        mut update_by: filters::UpdateArticleFilter<'_>,
        ...
        let query_res = db::article::delete_article(&self.conn, 
                                                    delete_by).await?;
    ...
    pub async fn favorite_article(&self, token: &str, favorite_by: filters::ArticleFilterByValues) -> Result<ArticleResponse, BackendError> {
        ...
        db::article::favorite_article(&self.conn, favorite_by, &claims.username).await
    }
    ...
    pub async fn unfavorite_article(&self, token: &str, unfavorite_by: filters::ArticleFilterByValues) -> Result<ArticleResponse, BackendError> {
        ...
        db::article::unfavorite_article(&self.conn, unfavorite_by, &claims.username).await
    }
    ...
    pub async fn add_comment(&self, 
        ...
        db::article::add_comment(&self.conn, article_filter, author, comment).await
    }
...
/*        let tmp = token_opt
            .and_then(|token| 
                auth::Auth::authenticate(token, self.secret.unwrap())
                    .ok()
...
*/    
 //       let author = &claims.username;
        ...
        let comments = db::article::get_comments(&self.conn, comments_filter, order_by, limit_offset)
            .await?;
    ...
    pub async fn delete_comment(&self, 
        ...
        let query_res = db::article::delete_comments(&self.conn, delete_by).await?;
        ...
        if 0 == query_res.rows_affected() {
            ...
            if let Some(id) = id_opt {
                ...
                let comments = db::article::get_comments(&self.conn, 
                                                        filter, 
                ...
                match comments {
                    Ok(comments) =>
                        if let Some(comment) = comments.iter().next() {
                            ...
                            auth::Auth::authorize(token, self.secret.unwrap(), &comment.comment.author)?;
    ...
    pub async fn get_tags(&self) -> Result<TagList, BackendError> {
        db::article::get_tags(&self.conn).await 
    }
...
impl Service for RegisterUserService {
    ...
    async fn execute(&self) -> Result<Self::ResultType, crate::errors::BackendError> {
        ...
        let res = match db::user::register_user(&self.conn, &self.user_reg).await {
            Ok(()) => {
